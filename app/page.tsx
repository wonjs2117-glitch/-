  ...
  return (
    <div className="bg-white min-h-screen font-serif text-[#1a1a1a] antialiased">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 mb-10 md:mb-14">
        <div className="border-y-2 border-black py-3 flex items-center gap-4 md:gap-8 text-[13px] md:text-[14px] font-sans">
          <span className="font-bold text-red-700 shrink-0 tracking-tighter border-r border-gray-300 pr-4 md:pr-6">속보</span>
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 pb-24">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10">
          ...
          <div className="md:col-span-9 grid grid-cols-1 md:grid-cols-2 gap-12 border-b md:border-b-0 md:border-r md:border-gray-200 md:pr-10 pb-10 md:pb-0">
            ...
            <div>
              <div className="border-b border-black mb-8 pb-1">
                <h3 className="text-xl font-bold font-sans tracking-tight">정치</h3>
              </div>
              ...
              <div className="space-y-12 md:space-y-16">
                {getNewsByCategory('정치').map((news) => (
                  <article key={news.id} className="group relative cursor-pointer" onClick={() => setSelectedNews(news)}>
                    ...
                    <h4 className="text-2xl md:text-[28px] font-bold leading-[1.2] mb-4 group-hover:text-gray-600 transition-colors">
                      {news.title}
                    </h4>
                    <p className="text-gray-600 text-[16px] md:text-[17px] leading-relaxed text-justify mb-5 line-clamp-3 font-serif">
                      {news.content}
                    </p>
                    <div className="text-[11px] font-sans font-bold text-gray-400 uppercase tracking-[0.15em]">
            ...
            <div className="md:border-l md:border-gray-100 md:pl-10">
              <div className="border-b border-black mb-8 pb-1">
                <h3 className="text-xl font-bold font-sans tracking-tight">경제</h3>
              </div>
          ...
          <div className="md:col-span-3">
            <div className="border-b border-black mb-8 pb-1">
              <h3 className="text-xl font-bold font-sans tracking-tight">오피니언</h3>
            </div>
            <div className="space-y-8">
              {getNewsByCategory('오피니언').map((news) => (
                <article key={news.id} className="group cursor-pointer border-b border-gray-100 pb-6 last:border-0" onClick={() => setSelectedNews(news)}>
                  <h4 className="text-[18px] font-bold leading-snug mb-2 group-hover:text-red-800 transition-colors italic">
                    "{news.title}"
                  </h4>
      ...
      <footer className="border-t border-black max-w-7xl mx-auto px-4 md:px-6 py-16 mb-20 mt-10">
        <div className="text-center">
          {/* 이 부분에 있던 'The Daily Major'가 삭제되었습니다. */}
          <div className="flex flex-wrap justify-center gap-x-8 md:gap-x-12 gap-y-4 text-[12px] md:text-[13px] font-sans font-bold text-gray-400 mb-12 uppercase">
      ...
      <button onClick={() => { if(checkAuth()) { setIsCreating(true); setEditForm({title:'', category:'정치', content:''}); } }} className="fixed bottom-8 right-8 md:bottom-12 md:right-12 w-14 h-14 bg-black text-white flex items-center justify-center text-2xl z-40 shadow-xl hover:scale-110 transition-transform rounded-full">✎</button>
          ...
          <div className="max-w-4xl mx-auto px-6 md:px-10 py-16 md:py-24 min-h-screen relative" onClick={(e) => e.stopPropagation()}>
            ...
            {isEditing || isCreating ? (
              <div className="flex flex-col gap-8 md:gap-10 font-serif">
                <textarea className="text-3xl md:text-4xl font-bold outline-none w-full h-32 resize-none border-b border-gray-100" value={editForm.title} onChange={e => setEditForm({...editForm, title: e.target.value})} placeholder="제목을 입력하세요" />
                <select className="font-sans font-bold text-xs md:text-sm uppercase border border-black w-40 p-2" value={editForm.category} onChange={e => setEditForm({...editForm, category: e.target.value})}>
                  <option value="정치">정치</option>
                  <option value="경제">경제</option>
                  <option value="사회">사회</option>
                  <option value="오피니언">오피니언</option>
                </select>
                <textarea className="h-[500px] md:h-[600px] text-lg md:text-xl leading-relaxed outline-none w-full" value={editForm.content} onChange={e => setEditForm({...editForm, content: e.target.value})} placeholder="본문 내용을 입력하세요..." />
                <div className="flex gap-4">
                  <button onClick={handleSave} className="flex-1 bg-black text-white py-4 md:py-5 font-sans font-bold uppercase text-sm tracking-widest">기사 발행</button>
                  {isEditing && (
                    <button onClick={() => handleDelete(selectedNews!.id)} className="bg-red-700 text-white px-8 md:px-12 font-sans font-bold uppercase text-sm">삭제</button>
                  )}
                </div>
              </div>
            ) : (
              selectedNews && (
                <article>
                  <div className="mb-12 md:mb-16 text-center border-b border-gray-100 pb-12 md:pb-16">
                    <span className="text-[11px] font-sans font-bold uppercase tracking-[0.2em] mb-6 md:mb-8 inline-block text-gray-500">{selectedNews.category}</span>
                    <h1 className="text-4xl md:text-5xl font-bold leading-tight mb-6 tracking-tight">{selectedNews.title}</h1>
                  ...
                  <div className="text-[#1a1a1a] leading-[1.9] md:leading-[2.1] text-justify whitespace-pre-line text-lg md:text-xl font-serif max-w-2xl mx-auto">
      ...
      {isBreakingMode && (
        <div className="fixed inset-0 bg-white/98 z-[100] flex justify-center items-center p-6">
          <div className="max-w-md w-full font-sans">
            <h2 className="text-xl font-bold mb-8 border-b border-black pb-2 italic uppercase tracking-tighter text-center">Newsroom Editor</h2>
            <div className="space-y-8 mb-12">
              {breakingNews.map((text, i) => (
                <div key={i}>
                  <p className="text-[10px] font-bold text-gray-400 mb-2 uppercase tracking-widest">속보 0{i+1}</p>
