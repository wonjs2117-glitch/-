            ...
            {isEditing || isCreating ? (
              <div className="flex flex-col gap-10 font-serif">
                ...
                <div className="bg-gray-50 p-6 border-l-4 border-gray-300">
                    <p className="text-xs font-sans font-black text-gray-400 uppercase mb-2">Editor's Note: 3줄 요약</p>
                    <p className="text-sm font-sans text-gray-500 leading-relaxed">본문의 가장 윗부분에 핵심 3줄을 적어주세요. 각 문장은 줄바꿈으로 구분됩니다.</p>
                </div>
                ...
                <textarea className="h-[500px] text-lg leading-relaxed outline-none w-full" value={editForm.content} onChange={e => setEditForm({...editForm, content: e.target.value})} placeholder="3줄 요약 작성 후 본문을 이어 작성하세요..." />
              ...
              selectedNews && (
                <article>
                  <div className="mb-10 text-center">
                    ...
                    {/* ✨ 3줄 요약 박스 */}
                    <div className="bg-[#f9f9f9] border-y border-gray-100 py-8 px-6 mb-12 text-left">
                        <h5 className="font-sans text-[11px] font-black text-gray-400 uppercase tracking-widest mb-4">핵심 요약 3줄</h5>
                        <ul className="list-disc list-inside space-y-3 font-sans text-[15px] font-bold leading-relaxed text-[#444]">
                            {selectedNews.content.split('\n').slice(0, 3).map((line, i) => (
                                <li key={i} className="marker:text-gray-300">{line}</li>
                            ))}
                        </ul>
                    </div>
                  </div>
